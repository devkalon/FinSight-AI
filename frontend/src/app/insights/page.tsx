  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1D263B] pb-6">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Financial Health & Insights Radar</h1>
      ...
      <div className="p-6 rounded-2xl bg-[#0F1626] border border-[#1D263B] space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1D263B] pb-4">
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 font-extrabold text-xl">
              {healthScore?.score || 78}
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="font-bold text-white text-lg">Composite Financial Health</h2>
              ...
              <p className="text-slate-400 text-xs">Deterministic scoring formula based on 7 weighted financial pillars.</p>
            </div>
          </div>

          <div className="text-right">
            <span className="text-[11px] text-slate-400 block">Score Range</span>
            <span className="font-bold text-white text-xs">78 / 100 • 85th Percentile</span>
          </div>
        </div>

        {/* 6 Factor Bars */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              <span>Savings Rate</span>
              <strong className="text-emerald-400">{c?.savings_rate?.score || 82}/100</strong>
            </div>
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              ...
              <strong className="text-blue-400">{c?.budget_adherence?.score || 75}/100</strong>
            </div>
            <div className="w-full h-1.5 rounded-full bg-[#182238] overflow-hidden">
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              ...
              <strong className="text-emerald-400">{c?.debt_burden?.score || 91}/100</strong>
            </div>
            <div className="w-full h-1.5 rounded-full bg-[#182238] overflow-hidden">
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              ...
              <strong className="text-amber-400">{c?.emergency_fund?.score || 63}/100</strong>
            </div>
            <div className="w-full h-1.5 rounded-full bg-[#182238] overflow-hidden">
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              ...
              <strong className="text-purple-400">{c?.spending_consistency?.score || 79}/100</strong>
            </div>
            <div className="w-full h-1.5 rounded-full bg-[#182238] overflow-hidden">
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2">
            <div className="flex justify-between font-semibold text-slate-300">
              ...
              <strong className="text-indigo-400">{c?.goal_progress?.score || 80}/100</strong>
            </div>
            <div className="w-full h-1.5 rounded-full bg-[#182238] overflow-hidden">
              <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${c?.goal_progress?.score || 80}%` }}></div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-2">
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-emerald-500/20 space-y-2">
            ...
            <ul className="space-y-1.5 text-slate-300">
              {(healthScore?.positive_factors || [
                'Excellent savings rate of 59.8% exceeds the 20% national benchmark.',
          ...
          <div className="p-4 rounded-xl bg-[#0A0E1A] border border-amber-500/20 space-y-2">
      ...
      <div className="p-6 rounded-2xl bg-[#0F1626] border border-[#1D263B] space-y-4">
        <div className="flex items-center justify-between border-b border-[#1D263B] pb-3">
          <div>
            ...
            <p className="text-slate-400 text-xs">Identifies category surges, transaction outliers, frequency spikes, and recurring hikes.</p>
          </div>
        ...
        {anomalies.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-400 bg-[#0A0E1A] rounded-xl border border-[#1D263B]">
          ...
          <div className="space-y-3">
            {anomalies.map((anom, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-[#0A0E1A] border border-[#1D263B] space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-amber-400 font-bold text-sm">+{anom.deviation_pct || 136}% Deviation</span>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 text-[11px]">
                  <div className="p-2 rounded-lg bg-[#12192B]">
                    <span className="text-slate-400 block">Observed Amount</span>
                    <strong className="text-white">{formatCurrency(anom.observed_value || 8450)}</strong>
                  </div>
                  <div className="p-2 rounded-lg bg-[#12192B]">
                    <span className="text-slate-400 block">Typical Expected</span>
                    <strong className="text-slate-300">{formatCurrency(anom.expected_value || 3580)}</strong>
                  </div>
                  <div className="p-2 rounded-lg bg-[#12192B] col-span-2">
                    <span className="text-slate-400 block">Explanation</span>
      ...
      <div className="p-5 rounded-2xl bg-[#0F1626] border border-blue-500/20 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
        <div className="flex items-center space-x-3">
          <div className="p-2.5 rounded-xl bg-blue-600/10 text-blue-400 border border-blue-500/20">
            <Sliders className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-bold text-white text-sm">Simulate "What-If" Financial Decisions</h3>
            <p className="text-slate-400 text-xs">Test salary raises, spending cuts, and subscription pruning deterministically.</p>
          </div>
        </div>
        <Link
          href="/simulator"
          className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold shadow-md shadow-blue-500/20 transition-all self-stretch sm:self-auto text-center"
        >
          Launch Simulator
        </Link>
      </div>
