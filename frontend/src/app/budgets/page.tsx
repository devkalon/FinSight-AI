  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Budgets & Envelope Limits</h1>
          <p className="text-slate-400 text-sm mt-0.5">
            Category thresholds, envelope allocation, real-time warning alerts & historical adherence
          </p>
        </div>
        <button
      ...
      {activeWarnings.length > 0 && (
        <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 space-y-2">
          <div className="flex items-center space-x-2.5 text-amber-400 font-bold text-xs">
            ...
            <span>Active Spending Threshold Alerts ({activeWarnings.length} categories)</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            {activeWarnings.map((wb) => (
              <div key={wb.id} className="p-2.5 rounded-xl bg-[#0D1322] border border-[#1E293B] flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <span className="font-semibold text-white">{wb.category?.name || 'Category'}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${wb.is_over_budget ? 'bg-rose-500/20 text-rose-400' : 'bg-amber-500/20 text-amber-400'}`}>
                    {wb.spent_percentage.toFixed(1)}%
                  </span>
                </div>
                <span className="text-slate-400">{formatCurrency(wb.spent_amount)} / {formatCurrency(wb.monthly_limit)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-6 rounded-2xl bg-[#0D1322] border border-[#1E293B] space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Monthly Budget Status</span>
            <div className="text-2xl font-extrabold text-white mt-1">
              {formatCurrency(totalSpent)} <span className="text-slate-500 text-base font-normal">/ {formatCurrency(totalLimit)}</span>
            </div>
          </div>
          <div className="sm:text-right">
            <span className={`text-sm font-bold ${overallPercentage > 100 ? 'text-rose-400' : overallPercentage > 80 ? 'text-amber-400' : 'text-emerald-400'}`}>
              {overallPercentage.toFixed(1)}% Overall Utilization
            </span>
        ...
        <div className="w-full h-3 rounded-full bg-[#1A2338] overflow-hidden">
          <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {budgets.map((b) => {
          ...
          return (
            <div key={b.id} className="p-5 rounded-2xl bg-[#0D1322] border border-[#1E293B] space-y-4 hover:border-slate-700 transition-all">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center font-bold"
                    style={{ backgroundColor: `${b.category?.color || '#3B82F6'}20`, color: b.category?.color || '#3B82F6' }}
                  >
                    <PiggyBank className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-sm">{b.category?.name || 'General Category'}</h3>
                    <span className="text-xs text-slate-400">Limit: {formatCurrency(b.monthly_limit)}/mo</span>
                  </div>
                </div>

                <div className="flex items-center space-x-2">
                  {isOver ? (
                    <span className="flex items-center space-x-1 text-xs text-rose-400 font-bold px-2.5 py-1 rounded-full bg-rose-500/10 border border-rose-500/20">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span>Over Budget</span>
                    </span>
                  ) : isWarning ? (
                    <span className="flex items-center space-x-1 text-xs text-amber-400 font-bold px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span>Warning ({b.spent_percentage.toFixed(0)}%)</span>
                    </span>
                  ) : (
                    <span className="flex items-center space-x-1 text-xs text-emerald-400 font-bold px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-[#11192C] transition-colors"
                  >
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-slate-300 font-semibold">{formatCurrency(b.spent_amount)} spent</span>
                  <span className="text-slate-400">{formatCurrency(b.remaining_amount)} left</span>
                </div>
                <div className="w-full h-2 rounded-full bg-[#1A2338] overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{
                      width: `${Math.min(b.spent_percentage, 100)}%`,
                      backgroundColor: isOver ? '#EF4444' : isWarning ? '#F59E0B' : (b.category?.color || '#3B82F6')
                    }}
                  ></div>
                </div>
              </div>
      ...
      <div className="p-6 rounded-2xl bg-[#0D1322] border border-[#1E293B] space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className="p-4 rounded-xl bg-[#11192C] border border-[#1E293B] space-y-2">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D1322] border border-[#1E293B] rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-white text-base">Create Category Budget</h3>
