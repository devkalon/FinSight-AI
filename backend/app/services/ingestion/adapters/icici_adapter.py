...
class ICICIBankAdapter(BankStatementAdapter):
    """
    Adapter for ICICI Bank Statement exports.
    """

    @property
    def name(self) -> str:
        return "ICICI Bank Statement Adapter"
    ...
    def matches_format(self, columns: List[str], sample_text: str = "") -> bool:
        ...
        has_tx_details = any("transaction details" in c or "details" in c for c in cols_lower)
        has_withdrawal_inr = any("withdrawal amount" in c or "withdrawal" in c for c in cols_lower)
        has_icici_text = "icici" in sample_text.lower()
        return (has_tx_details and has_withdrawal_inr) or has_icici_text

    def parse_dataframe(self, df: pd.DataFrame) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        results = []
        df.columns = [str(c).strip().lower() for c in df.columns]

        date_col = self.find_matching_column(df.columns, ["transaction date", "value date", "date"])
        desc_col = self.find_matching_column(df.columns, ["transaction details", "particulars", "description", "details"])
        ref_col = self.find_matching_column(df.columns, ["cheque number", "chq", "ref", "utr"])
        debit_col = self.find_matching_column(df.columns, ["withdrawal amount (inr )", "withdrawal amount", "withdrawal", "debit", "dr"])
        credit_col = self.find_matching_column(df.columns, ["deposit amount (inr )", "deposit amount", "deposit", "credit", "cr"])
        bal_col = self.find_matching_column(df.columns, ["balance (inr )", "balance", "bal"])
        ...
        for idx, row in df.iterrows():
            ...
            raw_desc = str(row[desc_col]).strip() if desc_col and pd.notna(row[desc_col]) else "ICICI Bank Transaction"
